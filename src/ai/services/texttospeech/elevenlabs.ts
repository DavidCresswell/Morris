...
import { prependWavHeader, readableToString } from "../../util";
...
export async function textToSpeech(inputStream: Readable): Promise<Readable> {
    if (settings.STREAM_ASSISTANT_TO_TTS) {
        return textToSpeechDualStreaming(inputStream);
    } else {
        let asString = await readableToString(inputStream);
        return await textToSpeechStreaming(asString);
    }
}
